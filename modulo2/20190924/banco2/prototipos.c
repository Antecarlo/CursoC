...
int pantalla2(char * nombre){
	...
	puts("5.CAMBIAR PIN");
	puts("6.DESCONECTAR");
	puts("7.BUSCAR MOVIMIENTOS");
	puts("8.BORRAR MOVIMIENTOS");
	puts("\nOPCION:");
...
void cambiarPin(TCliente * clientes, int posCliente){
	...
	do{
		if(clientes[posCliente].pin==pinEsc){
			...
			printf("\nPor favor, introduzca su pin actual, intento %d/3",n);
			pinEsc=atoi(fgets(aux,20,stdin));
			n++;
		}
	}while(clientes[posCliente].pin!=pinEsc);
}

/////////////////////////////////////////////////////////////////////////////////////////////////////

//BORRAR
void borrarMovClientes(TCliente * clientes, int numClientes){
...
void buscarMov(TCliente * clientes,int posCliente){
	int minutos,flag;
	char aux[20];

	printf("\n\nBUSCAR MOVIMIENTO:\n");
	printf("\nIntroduzca los minutos del movimiento a buscar:\n");
	fgets(aux,20,stdin);
	aux[2]='\0';
	minutos=atoi(aux);
	flag=buscarMovimiento(clientes[posCliente],minutos);
	
	if(flag!=-1){
		printf("\nMovimiento encontrado");
		imprimirMovimientoUnico(clientes[posCliente].movimientos, flag);
		
	}else{
		printf("\nEl movimiento buscado no se encuentra en la lista");
	}
