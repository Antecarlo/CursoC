...
TNodo * borrarR2 (TNodo * lista){

	if(lista!=NULL){
		lista->sig=borrarR2(lista->sig);
		free(lista);
		lista=NULL;
	}
	return lista;
...
void insertarRN (TNodo ** lista,TCoor c,int n){
	//n numero de coordenadas a añadir
	...
	if(* lista==NULL){
		...
		* lista=nuevo;
		if(n!=1){	
			insertarRN( &((* lista)->sig),c,n-1);
		}
	
	}else{
		insertarRN( &((* lista)->sig),c,n);
	}
}

void insertarR (TNodo ** lista,TCoor c){
	...
	if(* lista==NULL){
		...
		* lista=nuevo;
		...
		insertarR( &((* lista)->sig),c);
	}
}

TNodo * insertarR2 (TNodo * lista, TCoor c){
	TNodo * nuevo;
	...
	if(lista==NULL){
		...
		lista->sig=insertarR2(lista->sig,c);
	}
	return lista;
}
...
*/
	...
	////////////////////////////////////////////////
	printf("\n\nBorrar recursivo:\n");
	//lista=borrarR2(lista);
	//borrarR3(&lista);
	imprimir(lista);

	TCoor c={6,7};

	insertarRN(&lista,c,3);
	imprimir(lista);
